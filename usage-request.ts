...
export async function requestBalance(
	...
	const headers = { ...(source.headers ?? {}), ...(interpolateObject(request.headers ?? {}, vars) as Record<string, string>) };
	...
	const response = await fetcher(url, {
		...
		headers,
		body: request.body === undefined ? undefined : JSON.stringify(interpolateObject(request.body, vars)),
		signal: combined,
	});
	if (!response.ok) throw new Error(`Balance API error (${response.status})`);
	return extractBalanceText(JSON.parse(await readBoundedBody(response)), extractor);
}
