...
function interpolate(value: string, vars: Record<string, string>): string {
	return value.replace(/\{\{(\w+)\}\}/g, (_m, key: string) => {
		const result = vars[key];
		if (!result) throw new Error(`Missing balance variable: ${key}`);
		return result;
	});
}

/** 余额提取：按 extractor 的 JSON 路径取余量并加上单位与缩放。 */
export function extractBalanceText(body: unknown, extractor: JsonObject): string {
