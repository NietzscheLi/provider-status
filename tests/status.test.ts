...
import { extractBalanceText } from "../usage-extractor.ts";
import { UsageService } from "../usage-service.ts";
import { reconcileProviders } from "../reconcile.ts";

test("extracts nested and calculated balances with scaling", () => {
  assert.equal(extractBalanceText({ data: { total: 10, used: 2 } }, { totalPath: "data.total", usedPath: "data.used", scale: 0.5, unit: "$" }), "$4");
  assert.equal(extractBalanceText({ items: [{ remaining: 3 }] }, { remainingPath: "items.0.remaining", unit: "USD " }), "USD 3");
});
...
test("validity firstDefined/fallback/errorPath gate invalid responses", () => {
	...
	assert.throws(
		() => extractBalanceText({ is_active: false, message: "no quota" }, { ...base, errorPath: "message", validity: { firstDefined: ["is_active", "isValid"] } }),
		/no quota/,
	);
	assert.equal(extractBalanceText({ is_active: true, remaining: 3 }, { ...base, validity: { firstDefined: ["is_active", "isValid"] } }), "$3");
	// 字段都不存在时用 fallback 决定；fallback 为真则继续提取。
	assert.equal(extractBalanceText({ remaining: 7 }, { ...base, validity: { firstDefined: ["is_active", "isValid"], fallback: true } }), "$7");
	assert.throws(
		() => extractBalanceText({ remaining: 7 }, { ...base, validity: { firstDefined: ["is_active", "isValid"], fallback: false } }),
		/Balance query failed/,
	);
	// errorPath 缺失时回退 errorFallback。
	assert.throws(
		() => extractBalanceText({ is_active: false }, { ...base, errorPath: "message", validity: { firstDefined: ["is_active"] } }),
		/Balance query failed/,
	);
	// errorPath 优先于 errorFallback（allTruthy 失败同样生效）。
	assert.throws(
		() => extractBalanceText({ success: false, message: "unauthorized" }, { ...base, errorPath: "message", errorFallback: "Balance query failed", validity: { allTruthy: ["success"] } }),
		/unauthorized/,
	);
});
