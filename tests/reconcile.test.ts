...
test("显式 rename 事件迁移 balance key 并记录 alias，重复对账幂等", async () => {
	...
	assert.equal((config.balances as Record<string, unknown>).new && (config.balances as Record<string, unknown>).old, undefined);
	assert.equal(readUsageMap(dir).aliases.new.from, "old");
